...
if LunaEclipse_Scripts:isCompatableWOWVersion() and LunaEclipse_Scripts:isRequiredOvaleVersion(addonTable.REQUIRED_OVALE_VERSION) then
	...
	do
		local name = "lunaeclipse_demonhunter_spells";
		local desc = "LunaEclipse: Demon Hunter spells";
		local code = [[
			...
			Define(blade_dance 188499)
				...
				SpellInfo(blade_dance fury=15 itemset=T20 itemcount=4)
			...
			Define(chaos_blades_buff 247938)
				SpellInfo(chaos_blades_buff duration=18)
			...
			Define(death_sweep 210152)
				...
				SpellInfo(death_sweep fury=15 itemset=T20 itemcount=4)
			...
			Define(eye_beam 198013)
				SpellInfo(eye_beam fury=50 cd=45)
			Define(felblade 232893)
				SpellInfo(felblade cd=15 tag=main cd_haste=melee fury=-30 pain=-20)
			Define(fel_barrage 211053)
				SpellInfo(fel_barrage cd=60)
				SpellAddBuff(fel_barrage fel_barrage_buff)
				SpellAddTargetDebuff(fel_barrage fel_barrage_debuff)
			Define(fel_barrage_buff 211053)
				SpellInfo(fel_barrage_buff duration=2)
			Define(fel_barrage_debuff 211053)
				SpellInfo(fel_barrage_debuff duration=2)
			...
			Define(fel_rush 195072)
				SpellInfo(fel_rush tag=main cd=10 max_charges=2)
			...
			Define(throw_glaive_havoc 185123)
				SpellInfo(throw_glaive_havoc max_charges=1 cd=10 cd_haste=melee)
				SpellInfo(throw_glaive_havoc max_charges=2 talent=master_of_the_glaive_talent)
			Define(vengeful_retreat 198793)
				SpellInfo(vengeful_retreat cd=25)
				SpellInfo(vengeful_retreat addcd=-10 talent=prepared_talent)
